...
def normalize_latex(source: str, *, preserve_braces: bool = False) -> str:
    ...
    normalized = _replace_fractions(source)
    normalized = (
        normalized.replace("\\left", "")
        .replace("\\right", "")
        .replace("\\cdot", "*")
        .replace("\\times", "*")
        .replace("^{\\prime}", "'")
...
def _tokenize(source: str) -> list[Token]:
    ...
    expanded: list[Token] = []
    for token in tokens:
        if expanded and _is_atom_end(expanded[-1]) and _is_atom_start(token):
...
class _ExpressionParser:
    def __init__(self, tokens: list[Token], *, allow_x: bool, allow_i: bool, max_power: int) -> None:
        self.tokens = tokens
...
def parse_expression(source: str, *, allow_x: bool, allow_i: bool = False, max_power: int = 12) -> sp.Expr:
    ...
    expression = _ExpressionParser(_tokenize(normalized), allow_x=allow_x, allow_i=allow_i, max_power=max_power).parse()
...
def _function_parts(source: str, prime: bool) -> str:
    normalized = normalize_latex(source).replace(" ", "")
    pattern = r"^f'\(x\)=(.+)$" if prime else r"^f\(x\)=(.+)$"
    match = re.fullmatch(pattern, normalized)
    if not match:
        expected = "f'(x) = …" if prime else "f(x) = …"
        raise MathSyntaxError(f"Use function notation like {expected}.")
    return match.group(1)


def parse_function(source: str) -> sp.Expr:
    expression = parse_expression(_function_parts(source, prime=False), allow_x=True)
    _validate_polynomial(expression, max_degree=12)
    return expression


def parse_derivative(source: str) -> sp.Expr:
    expression = parse_expression(_function_parts(source, prime=True), allow_x=True)
    _validate_polynomial(expression, max_degree=12)
    return expression


def _validate_polynomial(expression: sp.Expr, *, max_degree: int) -> None:
    if sp.denom(sp.cancel(expression)).has(X):
        raise MathSyntaxError("Variable denominators are outside the current calculus scope.")
    try:
        degree = sp.Poly(expression, X).degree()
    except sp.PolynomialError as error:
        raise MathSyntaxError("Only polynomial expressions are supported.") from error
    if degree > max_degree:
